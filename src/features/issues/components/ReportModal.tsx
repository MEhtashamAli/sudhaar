...
import { 
  ...
  Trash2, Plus // Added Trash2 and Plus
} from 'lucide-react';
...
export default function ReportModal({ isOpen, onClose }: ReportModalProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    locationName: '', 
    coordinates: '',
    images: [] as File[] // CHANGED: Now an array of files
  });

  const handleClose = () => {
    setStep(1);
    setIsSubmitting(false);
    setFormData({ category: '', title: '', description: '', locationName: '', coordinates: '', images: [] });
    setMarkerPos(null);
    setSearchQuery("");
    onClose();
  };
  ...
  // ... (Search, LocateMe, and MapClick functions remain exactly the same) ...
                ...
                <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-8">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="flex gap-2 relative z-10">
                        ...
                        <button onClick={handleLocateMe} disabled={isLocating} className="px-4 bg-white border border-slate-200 text-slate-700 rounded-xl hover:bg-slate-50 hover:border-blue-300 hover:text-blue-600 transition-all flex items-center justify-center">
                            {isLocating ? <Loader2 className="animate-spin h-5 w-5" /> : <Crosshair className="h-5 w-5" />}
                        </button>
                        <button onClick={handleSearch} disabled={isSearching} className="px-6 bg-slate-900 text-white font-bold rounded-xl text-sm hover:bg-slate-800 disabled:opacity-70 transition-colors flex items-center justify-center">
                  ...
                  {/* --- UPDATED IMAGE UPLOAD SECTION (MULTIPLE IMAGES) --- */}
                  <div className="space-y-4">
                    <label className="text-sm font-bold text-slate-700 uppercase tracking-wider flex justify-between">
                        <span>5. Evidence Photos</span>
                        <span className="text-xs normal-case text-slate-400">{formData.images.length}/3 Uploaded</span>
                    </label>
                    ...
                    {formData.images.length === 0 ? (
                        ...
                        <div 
                            ...
                            className="relative w-full h-32 rounded-2xl border-2 border-dashed border-slate-300 bg-slate-50 hover:bg-white hover:border-blue-400 flex flex-col items-center justify-center cursor-pointer transition-all group"
                        >
                            <div className="h-10 w-10 bg-white text-blue-500 rounded-xl shadow-sm flex items-center justify-center mb-2 group-hover:scale-110 transition-transform">
                                <Camera size={20} />
                            </div>
                            <p className="font-bold text-slate-600 text-sm">Click to Upload Photos</p>
                            <p className="text-xs text-slate-400 mt-1">Supports JPG, PNG (Max 3)</p>
                        </div>
                    ) : (
                        ...
                        <div className="grid grid-cols-4 gap-3">
                            {formData.images.map((file, index) => (
                                <div key={index} className="relative aspect-square rounded-xl overflow-hidden border border-slate-200 group">
                                    <img 
                                        ...
                                        onLoad={(e) => URL.revokeObjectURL(e.currentTarget.src)} // Memory cleanup
                                    />
                                    <button 
                            ...
                            {formData.images.length < 3 && (
                                <button 
                                    onClick={() => fileInputRef.current?.click()}
                                    className="aspect-square rounded-xl border-2 border-dashed border-slate-300 bg-slate-50 hover:bg-white hover:border-blue-400 flex flex-col items-center justify-center cursor-pointer transition-all text-slate-400 hover:text-blue-500"
                                >
                                    <Plus size={24} />
            ...
            {step < 3 && (
              <div className="px-8 py-6 bg-slate-50 border-t border-slate-200 flex justify-between items-center sticky bottom-0 z-20">
                ...
                <button 
                  ...
                  // UPDATED DISABLED LOGIC FOR STEP 2 (Check array length)
                  disabled={step === 1 ? (!formData.title || !formData.category) : (!formData.coordinates || formData.images.length === 0 || isSubmitting)} 
